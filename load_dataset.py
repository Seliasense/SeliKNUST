...
if config.ModelDatasetSel == 5 :
    ...
    from torch_geometric.loader import DataLoader  # added this line
else:
    from torch_geometric.data import Data, InMemoryDataset, DataLoader
...
class MUTAGDataset(InMemoryDataset):
    def __init__(self, root, name, transform=None, pre_transform=None):
        ...
        self.data, self.slices = torch.load(self.processed_paths[0], weights_only=False) # we added weight_only=false
...
class SentiGraphDataset(InMemoryDataset):
    def __init__(self, root, name, transform=None, pre_transform=undirected_graph):
        ...
        self.data, self.slices, self.supplement = torch.load(self.processed_paths[0], weights_only= False) # we added weight_only=false
