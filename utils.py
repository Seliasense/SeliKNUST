...
def find_closest_node_result(results, max_nodes):
    """
    Main function that selects between sub-functions
    strategies: 2 (MCTS) for original  or  1 for modified ( same coalition sizes)
    """
    if config.strategy == 'MCTS':
        return find_by_Original_Method(results, max_nodes)
    else:  # strategy == 'size'
        return find_by_Modified_Method(results, max_nodes)



def find_by_Original_Method(results, max_nodes):
...
def find_by_Modified_Method(results, max_nodes):
    
    """Return the highest reward tree_node whose subgraph is smaller than max_nodes"""
    
    # Filter out items with no coalition or empty coalition
    valid_results = [x for x in results if hasattr(x, 'coalition') and x.coalition is not None and len(x.coalition) > 0]
    
    # Check if we have any valid results after filtering
    if  valid_results:
        
        # Sort by coalition size (ascending) and then by reward (descending) for same sizes
        valid_results = sorted(valid_results, key=lambda x: (len(x.coalition), -x.P))
        result_node = None
        highest_reward = float('-inf')
        
        for x in valid_results:
            if len(x.coalition) <= max_nodes:
                # This is a valid candidate - check if it has higher reward
                if x.P > highest_reward:
                    result_node = x
                    highest_reward = x.P
                           
        # If no valid node found under max_nodes, return the smallest one as fallback
        if result_node is None:
            result_node = valid_results[0]
...
class PlotUtils():
    ...
    def plot(self, graph, nodelist, figname, **kwargs):
        ...
        # elif self.dataset_name.lower() in ['grt_sst2_BERT_Identity'.lower()]: # commented out
        elif self.dataset_name.lower() in ['graph_sst2'.lower()]: # added this line
